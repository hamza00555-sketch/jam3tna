...
## الحالة الحالية: M2.5 — مؤقّتان + شاشة العيد

**M1** ✅ — التهيئة + Theme + Routing + Auth + SectionPicker.

**M2** ✅ — Home grid + Bring List هجين + إدارة الفئات.

**M2.5** ✅ — مؤقّتان + شاشة عيد + Admin Settings:
- **مؤقّتان منفصلان** في Home: يوم العيد + يوم الاستراحة.
- **شاشة "عيد مبارك"** تُفتح تلقائياً يوم العيد (مرّة واحدة عبر SharedPreferences):
  - ألعاب نارية حقيقية (`CustomPaint` + Ticker + Particles مع gravity).
  - تكبيرات تشتغل تلقائياً من `assets/audio/takbeer.mp3` بـ loop (`just_audio`).
  - تهنئة كبيرة بـ shader gold gradient + خلفية radial داكنة + نجوم.
  - زر مكتوم/مفتوح للصوت + زر إغلاق.
  - يفشل بهدوء إذا لم يكن ملف الصوت موجوداً (يعرض إشعاراً خفيفاً).
- **AdminSettings**: شاشة لتعديل تاريخ العيد + تاريخ الاستراحة + الموقع + إحداثيات.
- **ميزات Bring List السابقة:**
  - فلاتر: قسمي / مشترك / الكل · تجميع بالفئة · transactions ضد race.
  - حجز/تعديل/إلغاء (للمالك) · إضافة بنود حرّة · AdminCategories.

### الوصول للشاشات (للأدمن)
- Home → ⚙️ زر **إعدادات اللمَّة** (التواريخ + الموقع).
- قائمة الجلب → ⚙️ زر **إعدادات الفئات** (الكوتا).
- بطاقة "اليوم عيد مبارك" تظهر في Home حين يوم العيد.
...
- ✅ **M2.5**: مؤقّتان + شاشة عيد بألعاب نارية + تكبيرات + Admin Settings.
